...
"#;
    ...
    let plan = cegar_refinement_plan_with_signals(&program, Some(&signals), SolverChoice::Z3, 5);
    assert!(
        !plan.is_empty(),
...
"#;
    ...
    let plan_a = cegar_refinement_plan_with_signals(&program, Some(&signals), SolverChoice::Z3, 5);
    let plan_b = cegar_refinement_plan_with_signals(&program, Some(&signals), SolverChoice::Z3, 5);
...
fn ltl_conformance_persistence_eventually_always() {
    ...
    let prop = make_property_decl(
        ...
        ast::PropertyKind::Liveness,
        vec![forall_binding("p", "R")],
        ast::FormulaExpr::Eventually(Box::new(ast::FormulaExpr::Always(Box::new(eq_comparison(
            "p", "stable", true,
        ))))),
    );
    let frag = classify_property_fragment(&prop).unwrap();
...
"#;
    ...
    let result = verify_all_properties(src, "temporal_failing.trs", &options).expect("verify all");
    let live = result
        .verdicts
        .iter()
...
"#;
    ...
    let ta = lower_with_active_controls(&program, "cti_concrete_classification").expect("lower");
