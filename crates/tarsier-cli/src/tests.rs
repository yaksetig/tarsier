use super::{
    ...
    lint_protocol_file, merge_portfolio_fair_liveness_results, merge_portfolio_liveness_results,
    merge_portfolio_prove_fair_results, merge_portfolio_prove_results,
    merge_portfolio_verify_reports, obligations_all_unsat, parse_cli_network_semantics_mode,
    parse_visualize_check, parse_visualize_format, prefer_trace_a, render_lint_text,
    render_phase_profile_summary, run_conformance_suite, run_diagnostics_details,
    run_portfolio_workers, sha256_hex_bytes, trace_fingerprint, trace_json,
    unbounded_fair_result_details, verification_result_kind, write_certificate_bundle,
    AnalysisConfig, AnalysisLayerReport, CertificateBundleInput, CertificateBundleObligation,
    CertificateKind, CertificateMetadata, CertificateObligationMeta, Cli, CliNetworkSemanticsMode,
    Commands, DebugFilter, FairnessMode, ProveAutoTarget, SoundnessMode, VisualizeCheck,
    VisualizeFormat, CERTIFICATE_SCHEMA_VERSION, CLI_LONG_ABOUT, CONFORMANCE_TRIAGE_CATEGORIES,
...
use tarsier_engine::pipeline::{
    AppliedReductionDiagnostic, AutomatonFootprint, PhaseProfileDiagnostic, PipelineRunDiagnostics,
    PropertyAssumptionsDiagnostic, PropertyCompilationDiagnostic, PropertyResultDiagnostic,
    PropertyTemporalMonitorStepDiagnostic, PropertyWitnessMetadataDiagnostic, SmtProfileDiagnostic,
};
...
fn write_certificate_bundle_is_canonical_for_equivalent_inputs() {
    ...
    write_certificate_bundle(&out_a, &input_a, false, false).expect("bundle a should be written");
    write_certificate_bundle(&out_b, &input_b, false, false).expect("bundle b should be written");
...
"#;
    let report = lint_protocol_file(src, "faithful_equivocation_lint.trs", SoundnessMode::Strict);
    let issue = report
        .issues
        .iter()
...
fn triage_classifies_model_change_first() {
    let triage =
        classify_cert_suite_check_triage("verify", "safe", "unsafe", Some("expected_safe"), true);
...
fn triage_classifies_expected_update_for_known_bug_polarity_match() {
    let triage =
        classify_cert_suite_check_triage("prove", "not_proved", "unsafe", Some("known_bug"), false);
...
fn cert_suite_corpus_breadth_passes_for_canonical_library_manifest() {
    ...
    let manifest: CertSuiteManifest = serde_json::from_str(&raw).expect("manifest should decode");
    let errors = validate_manifest_corpus_breadth(&manifest, &manifest_path);
    assert!(
        errors.is_empty(),
...
"#;
    let target = detect_prove_auto_target(src, "mixed_props.trs").expect("parse should succeed");
...
fn trusted_check_requires_proof_checker_by_default() {
    ...
    let solvers = vec!["z3".to_string(), "cvc5".to_string()];
    let err = validate_trusted_check_requirements(true, 2, &solvers, &metadata, true, None, false)
...
fn unbounded_fair_unknown_details_include_stable_reason_codes() {
    let timeout_details = unbounded_fair_result_details(&UnboundedFairLivenessResult::Unknown {
...
fn conformance_manifest_schema_json_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../docs/conformance-manifest-schema-v1.json")
}
...
fn trust_report_schema_doc_exists() {
    ...
    let content = fs::read_to_string(&path).expect("trust report schema doc should be readable");
    assert!(
        content.contains("schema_version"),
        "doc should mention schema_version"
    );
...
fn capture_proofs_strict_fails_on_missing_proof() {
    ...
    assert!(
        err_msg.contains("Proof extraction failed") || err_msg.contains("--allow-missing-proofs"),
...
fn capture_proofs_with_allow_missing_continues() {
    ...
    let cert_json =
        fs::read_to_string(out.join("certificate.json")).expect("certificate.json should exist");
...
fn conformance_suite_json_output_includes_artifact_links() {
    ...
    let tmp = std::env::temp_dir().join(format!("tarsier_conformance_json_{}", std::process::id()));
    let _ = fs::remove_dir_all(&tmp);
...
fn conformance_suite_mismatch_taxonomy_uses_model_impl_and_engine_categories() {
    ...
    let fail_trace =
        workspace_root.join("crates/tarsier-conformance/tests/fixtures/guard_not_satisfied.json");
...
fn governance_pipeline_report_schema_v1_structure() {
    ...
    let json: serde_json::Value = serde_json::to_value(&report).expect("serialize pipeline report");
