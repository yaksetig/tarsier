...
fn go_semantic_validation_matches_model_oracle_for_core_examples() {
    ...
    for (name, source) in fixtures {
        ...
        let generated = generate(&program, CodegenTarget::Go).expect("go codegen should work");
        let report = validate_generated_trace_oracle(&oracle, CodegenTarget::Go, &generated);
        assert!(
            report.is_match(),
...
#[test]
fn go_semantic_validation_flags_missing_send_semantics() {
    ...
    let broken = generated.replacen(
        "channelAuthForMessageFamily(\"Echo\")",
