...
impl InMemoryNetworkShim {
    ...
    fn apply_fault(&mut self, action: &FaultAction) -> Result<(), NetworkShimError> {
        match action {
...
mod tests {
    ...
    fn partition_blocks_until_healed() {
        ...
        shim.apply_fault_at_tick(1, &FaultAction::HealPartition)
            .unwrap();
        let delivered = shim.drain_deliverable();
        assert_eq!(delivered.len(), 1);
        assert_eq!(delivered[0].message_id, 1);
    ...
    fn spawn_and_retire_twin_control_message_duplication() {
        ...
        shim.apply_fault_at_tick(0, &FaultAction::RetireTwin { twin_id: 50 })
            .unwrap();
    ...
    fn apply_fault_at_tick_rejects_tick_regression() {
        let mut shim = InMemoryNetworkShim::new();
        shim.apply_fault_at_tick(5, &FaultAction::HealPartition)
            .unwrap();
        let err = shim
            .apply_fault_at_tick(4, &FaultAction::HealPartition)
            .unwrap_err();
